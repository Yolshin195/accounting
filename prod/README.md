...
```
Обе команды сами ставят и включают `cron`, если его нет на сервере, и `install-auto-update` сразу проверяет, что автообновление реально отрабатывает (запускает `auto-update` один раз вручную) — так что для автоматики достаточно выполнить только эти команды, ничего больше настраивать не нужно.
